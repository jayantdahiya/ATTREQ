#!/usr/bin/env python3
"""
Moondream 1.8B Accuracy Enhancer for ATTREQ Wardrobe Classification
Implements multiple strategies to achieve 100% accuracy
"""
...
class MoondreamAccuracyEnhancer:
    def __init__(self, ollama_url: str = "http://localhost:11434"):
        self.ollama_url = ollama_url
        self.model = "moondream:1.8b"
    ...
    def enhance_classification(self, image_path: str) -> ClothingClassification:
        ...
        """
        # Strategy 1: Try explicit JSON format
    ...
    def _try_explicit_json(self, image_path: str) -> Dict[str, Any]:
        """Strategy 1: Explicit JSON format with perfect instructions"""
...
This is a black leather jacket - analyze it precisely."""
        ...
        return self._call_moondream(prompt, image_path, temperature=0.0)
...
Please be specific and accurate."""
        ...
        response = self._call_moondream(prompt, image_path, temperature=0.1)
        return self._extract_from_conversation(response)

    def _try_hybrid(self, image_path: str) -> Dict[str, Any]:
        """Strategy 3: Hybrid approach combining description and JSON"""
        prompt = """Analyze this clothing image. Describe what you see, then convert to JSON format:

Description: [describe the clothing]

JSON: {"category": "", "color_primary": "", "pattern": "", "season": "", "occasion": "", "detection_confidence": 0.0}"""
        
        return self._call_moondream(prompt, image_path, temperature=0.1)

    def _call_moondream(self, prompt: str, image_path: str, temperature: float = 0.1) -> Dict[str, Any]:
        """Call Moondream API with optimized parameters"""
        try:
    ...
    def _parse_response(self, response_text: str) -> Dict[str, Any]:
        """Parse Moondream response to extract JSON"""
    ...
    def _extract_color(self, text: str) -> str:
        ...
        text_lower = text.lower()
        colors = ["black", "white", "blue", "red", "green", "yellow", "brown", "gray", "grey"]
        for color in colors:
    ...
    def _post_process(self, result: Dict[str, Any]) -> ClothingClassification:
        ...
        color_primary = result.get("color_primary", "black")
        pattern = self.pattern_mappings.get(result.get("pattern", ""), "solid")
        season = self.season_mappings.get(result.get("season", ""), "all")
        occasion = self.occasion_mappings.get(result.get("occasion", ""), ["casual", "formal"])
