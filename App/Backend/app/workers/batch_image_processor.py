...
from app.services.ai.background_removal import background_removal_service
...
async def process_batch_wardrobe_images(
    ...
    logger.info(f"Completed batch processing: {processed_count}/{len(image_paths)} images processed")
...
async def _process_single_batch(
    ...
    try:
        ...
        classification_results = await gemini_classifier_service.classify_batch_images(batch_image_paths)
        
        # Ensure we have results for all images
        if len(classification_results) != len(batch_image_paths):
            logger.warning(f"Gemini returned {len(classification_results)} results for {len(batch_image_paths)} images")
        ...
        for i, (item_id, image_path, classification_result) in enumerate(
            zip(batch_item_ids, batch_image_paths, classification_results)
        ):
            try:
...
async def _fallback_to_individual_processing(
    ...
    for item_id, image_path in zip(batch_item_ids, batch_image_paths):
        try:
            # Use single image classification
            classification_result = await gemini_classifier_service.classify_single_image(image_path)
