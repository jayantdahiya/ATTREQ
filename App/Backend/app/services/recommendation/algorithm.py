...
async def filter_items_by_occasion(
    items: list[WardrobeItem], occasion: str
) -> list[WardrobeItem]:
...
async def get_recently_worn_items(
    db: AsyncSession, user_id: UUID, days: int = 14
) -> set[UUID]:
...
async def get_user_preference_weights(db: AsyncSession, user_id: UUID) -> dict[str, Any]:
    ...
    query = select(Outfit).where(
        and_(Outfit.user_id == user_id, Outfit.feedback_score == 1)
    )
...
async def generate_daily_outfits(
    ...
    if len(all_items) < 2:
        logger.warning(f"User {user_id} has insufficient items ({len(all_items)}) for outfit generation")
        return []
    ...
    accessories = [
        ...
        if item.category and any(acc in item.category.lower() for acc in ["accessory", "shoe", "bag"])
    ]
    ...
    logger.info(f"Available items: {len(tops)} tops, {len(bottoms)} bottoms, {len(accessories)} accessories")
    ...
    for bottom in bottoms:
        ...
        for top in tops:
            ...
            if user_preferences["preferred_colors"]:
                if top.color_primary and top.color_primary.lower() in user_preferences["preferred_colors"]:
                    preference_bonus += 0.1
                if bottom.color_primary and bottom.color_primary.lower() in user_preferences["preferred_colors"]:
                    preference_bonus += 0.1
                    ...
                    "accessory_item": (
                        {
                            ...
                            "image_url": accessory.processed_image_url or accessory.original_image_url,
