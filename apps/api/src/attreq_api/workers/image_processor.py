...
async def process_wardrobe_image(item_id: UUID, user_id: UUID, original_image_path: str) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            # Step 2 & 3: Remove background + generate thumbnail (shared helper)
            processed_image_path, processed_image_url, thumbnail_url = (
                await generate_processed_and_thumbnail(
                    original_image_path, user_id, log_ref=f"item {item_id}"
                )
            )
...
async def process_wardrobe_item_photo(
    ...
    Args:
        ...
        original_image_path: Path to the original uploaded photo
    """
    logger.info(f"Starting photo processing for photo {photo_id}")

    async with AsyncSessionLocal() as db:
        try:
            _, processed_image_url, thumbnail_url = await generate_processed_and_thumbnail(
