...
export function WardrobeStack() {
  ...
  // Hardware back mirrors the on-screen back: sub-screens return to the list;
  // at the list (stack root) fall through to the default (exit).
  useBackHandler(() => {
    if (screen.name === 'list') return false;
    setScreen({ name: 'list' });
    return true;
  });
